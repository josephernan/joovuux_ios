...
@interface ModeSettingsVC () <UITableViewDelegate, UITableViewDataSource>
{
    BOOL fieldOfViewHDR;
    BOOL fieldOfViewSEC;
    ...
    __weak IBOutlet UITableViewCell *timeLaoseCell;
...
- (void)viewDidLoad {
    ...
    [self.resetAllSettingsForMode1 addTarget:self action:@selector(resetSettingsMode1ButtonHighlight:) forControlEvents:UIControlEventTouchDown];
    [self.resetAllSettingsForMode1 addTarget:self action:@selector(resetSettingsMode1Button:) forControlEvents:UIControlEventTouchDragOutside];
    ...
    //if ([[NSUserDefaults standardUserDefaults] boolForKey:@"firstOpenModeOneKEY"]) {
...
-(void) saveAllSettings
{
    if ([self.appDelegate isConnected]) {
        if ([[[CameraManager cameraManager] allSettingsDict] isKindOfClass:[NSMutableDictionary class]]) {
            ...
            dict = @[
                     @{@"video_resolution":self.resolutionDesctinationLabel.text},
                     @{@"video_quality":videoBit},
                     @{@"video_length":self.videoClipDestinationLabel.text},
                     @{@"fleld_view":self.fieldOfViewDestinationLabel.text},
                     @{@"timelapse_video":self.timeLapsDestinationLabel.text},
                     @{@"audio":audioCheck?@"on":@"off"},
                     @{@"video_timestamp":videoTimeStampCheck?@"on":@"off"},
                     @{@"rotate_video":imageRotationLabel.text},
                     @{@"loop_record":loopRecordingCheck?@"off":@"on"},
                     @{@"wdr":wdrCheck?@"off":@"on"}
                     ];
            
            [[[CameraManager cameraManager] allSettingsDict] setObject:dict forKey:@"modeOne"];
        }
    }
}
...
-(void) loadAllSettings
{
    ...
    CameraManager *camera_manager = [CameraManager cameraManager];
    if(camera_manager.allSettingsDict.count==0)
        [camera_manager getSettingsFromCamera];
    array = [camera_manager.allSettingsDict valueForKey:@"modeOne"];
    ...
    self.resolutionDesctinationLabel.text = [settings valueForKey:@"video_resolution"];
    
    
    if ([[settings valueForKey:@"video_quality"] isEqualToString:@"S.Fine"])
    {
        self.videoBitDestinationLabel.text = @"High";
    }
    else if ([[settings valueForKey:@"video_quality"] isEqualToString:@"Fine"])
    {
        self.videoBitDestinationLabel.text = @"Medium";
    }
    else if ([[settings valueForKey:@"video_quality"] isEqualToString:@"Normal"])
    {
        self.videoBitDestinationLabel.text = @"Low";
    }
    
    self.videoClipDestinationLabel.text = [settings valueForKey:@"video_length"];
    self.fieldOfViewDestinationLabel.text = [settings valueForKey:@"fleld_view"];
    
    NSString * replaceStr = [settings valueForKey:@"timelapse_video"];
    replaceStr = [replaceStr stringByReplacingOccurrencesOfString:@"_" withString:@" "];
    self.timeLapsDestinationLabel.text = replaceStr;

    
    if (![[settings valueForKey:@"audio"] isEqualToString:@"on"]) {
        audioCheck = NO;
    }
    else
    {
    ...
    if (![[settings valueForKey:@"video_timestamp"] isEqualToString:@"on"]) {
    ...
    imageRotationLabel.text = [settings valueForKey:@"rotate_video"];
    
    if (![[settings valueForKey:@"loop_record"] isEqualToString:@"on"]) {
    ...
    if (![[settings valueForKey:@"wdr"] isEqualToString:@"on"]) {
        wdrCheck = YES;
    }
    else
    {
...
- (IBAction)audioAction:(UIButton *)sender {
    ...
    if (!audioCheck) {
            [[CameraManager cameraManager] params:@"on" type:@"audio"];
        [self.audio setTitle:@"ON" forState:UIControlStateNormal];
        [self.audio setBackgroundColor:[UIColor colorWithRed:72/256.0 green:216/256.0 blue:183/256.0 alpha:1]];
        audioCheck = YES;
        
    } else {
        [[CameraManager cameraManager] params:@"off" type:@"audio"];
        [self.audio setTitle:@"OFF" forState:UIControlStateNormal];
        [self.audio setBackgroundColor:[UIColor colorWithRed:166/256.0 green:166/256.0 blue:166/256.0 alpha:1]];
         audioCheck = NO;
    }
...
- (IBAction)videoTimeStampButton:(UIButton *)sender {
    if (!videoTimeStampCheck) {
        [[CameraManager cameraManager] params:@"on" type:@"video_timestamp"];
        [self.videoTimeStamp setTitle:@"ON" forState:UIControlStateNormal];
        [self.videoTimeStamp setBackgroundColor:[UIColor colorWithRed:72/256.0 green:216/256.0 blue:183/256.0 alpha:1]];
        videoTimeStampCheck = YES;
        ...
        [[CameraManager cameraManager] params:@"off" type:@"video_timestamp"];
        [self.videoTimeStamp setTitle:@"OFF" forState:UIControlStateNormal];
        [self.videoTimeStamp setBackgroundColor:[UIColor colorWithRed:166/256.0 green:166/256.0 blue:166/256.0 alpha:1]];
         videoTimeStampCheck = NO;
...
//    if (!rotateDegreesCheck) {
//     [[CameraManager cameraManager] params:@"on" type:@"rotate_video"];
//    [self.rotateDegrees setTitle:@"ON" forState:UIControlStateNormal];
//    [self.rotateDegrees setBackgroundColor:[UIColor colorWithRed:72/256.0 green:216/256.0 blue:183/256.0 alpha:1]];
//    rotateDegreesCheck = YES;
//        
//    } else {
//     [[CameraManager cameraManager] params:@"off" type:@"rotate_video"];
//    [self.rotateDegrees setTitle:@"OFF" forState:UIControlStateNormal];
//    [self.rotateDegrees setBackgroundColor:[UIColor colorWithRed:166/256.0 green:166/256.0 blue:166/256.0 alpha:1]];
...
- (IBAction)loopRecordingButton:(UIButton *)sender {
    if (!loopRecordingCheck) {
        [[CameraManager cameraManager] params:@"off" type:@"loop_record"];
        [self cell:self.VideoClipCell setHidden:YES];
        [self reloadDataAnimated:YES];
        [self.loopRecording setTitle:@"OFF" forState:UIControlStateNormal];
        [self.loopRecording setBackgroundColor:[UIColor colorWithRed:166/256.0 green:166/256.0 blue:166/256.0 alpha:1]];
        ...
        [[CameraManager cameraManager] params:@"on" type:@"loop_record"];
        [self cell:self.VideoClipCell setHidden:NO];
        [self reloadDataAnimated:YES];
        loopRecordingCheck = NO;
        [self.loopRecording setTitle:@"ON" forState:UIControlStateNormal];
...
- (IBAction)wdrButton:(UIButton *)sender {
    ...
    if (!wdrCheck) {
        [[CameraManager cameraManager] params:@"off" type:@"wdr"];
        [self.wdr setTitle:@"OFF" forState:UIControlStateNormal];
        [self.wdr setBackgroundColor:[UIColor colorWithRed:166/256.0 green:166/256.0 blue:166/256.0 alpha:1]];
        wdrCheck = YES;
        
    } else {
         [[CameraManager cameraManager] params:@"on" type:@"wdr"];
        wdrCheck = NO;
        [self.wdr setTitle:@"ON" forState:UIControlStateNormal];
        [self.wdr setBackgroundColor:[UIColor colorWithRed:72/256.0 green:216/256.0 blue:183/256.0 alpha:1]];
    }
...
- (IBAction)resolution1Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"2560x1080 30P 21:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
- (IBAction)resolution2Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [self hideWDRAndTimeLapseAndFieldOfView:NO];
    [[CameraManager cameraManager] params:@"2304x1296 30P 16:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
- (IBAction)resolution3Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [self hideWDRAndTimeLapseAndFieldOfView:NO];
    [[CameraManager cameraManager] params:@"1920x1080 60P 16:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
- (IBAction)resolution5Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [self hideWDRAndTimeLapseAndFieldOfView:NO];
    [[CameraManager cameraManager] params:@"1920x1080 45P 16:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
- (IBAction)resolution6Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [self hideWDRAndTimeLapseAndFieldOfView:NO];
     [[CameraManager cameraManager] params:@"1920x1080 30P 16:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
- (IBAction)resolution7Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"HDR 1920x1080 30P 16:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
- (IBAction)resolution8Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [self hideWDRAndTimeLapseAndFieldOfView:YES];
    [[CameraManager cameraManager] params:@"1280x720 60P 16:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
- (IBAction)resolution9Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [self hideWDRAndTimeLapseAndFieldOfView:NO];
     [[CameraManager cameraManager] params:@"1280x720 30P 16:9" type:@"video_resolution"];
    for (UITableViewCell * cell in self.videoResolutionCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoResolutionCheck = YES;
...
-(void) hideWDRAndTimeLapseAndFieldOfView:(BOOL)hide
{
    ...
    [self cell:timeLaoseCell setHidden:hide];
...
- (IBAction)videoBit1Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"S.Fine" type:@"video_quality"];
    for (UITableViewCell * cell in self.videoBitRatesCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoBitRatesCheck = YES;
...
- (IBAction)videoBit2Button:(UIButton *)sender {
    self.videoBitDestinationLabel.text = self.videoBit2Label.text;
     [[CameraManager cameraManager] params:@"Fine" type:@"video_quality"];
    for (UITableViewCell * cell in self.videoBitRatesCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoBitRatesCheck = YES;
...
- (IBAction)videoBit3Button:(UIButton *)sender {
    self.videoBitDestinationLabel.text = self.videoBit3Label.text;
     [[CameraManager cameraManager] params:@"Normal" type:@"video_quality"];
    for (UITableViewCell * cell in self.videoBitRatesCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoBitRatesCheck = YES;
...
- (IBAction)fieldOfView1Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"155" type:@"fleld_view"];
    for (UITableViewCell * cell in self.fieldOfViewCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        fieldOfViewCheck = YES;
...
- (IBAction)fieldOfView2Button:(UIButton *)sender {
    self.fieldOfViewDestinationLabel.text = self.fieldOfView2Label.text;
      [[CameraManager cameraManager] params:@"120" type:@"fleld_view"];
    for (UITableViewCell * cell in self.fieldOfViewCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        fieldOfViewCheck = YES;
...
- (IBAction)fieldOfView3Button:(UIButton *)sender {
    self.fieldOfViewDestinationLabel.text = self.fieldOfView3Label.text;
      [[CameraManager cameraManager] params:@"90" type:@"fleld_view"];
    for (UITableViewCell * cell in self.fieldOfViewCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        fieldOfViewCheck = YES;
...
- (IBAction)fieldOfView4Button:(UIButton *)sender {
    self.fieldOfViewDestinationLabel.text = self.fieldOfView4Label.text;
      [[CameraManager cameraManager] params:@"60" type:@"fleld_view"];
    for (UITableViewCell * cell in self.fieldOfViewCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        fieldOfViewCheck = YES;
...
- (IBAction)timeLaps1Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [[CameraManager cameraManager] params:@"off" type:@"timelapse_video"];
    for (UITableViewCell * cell in self.timeLapseCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        timeLapseCheck = YES;
...
- (IBAction)timeLaps2Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
     [[CameraManager cameraManager] params:@"1_sec" type:@"timelapse_video"];
    for (UITableViewCell * cell in self.timeLapseCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        timeLapseCheck = YES;
...
- (IBAction)timeLaps3Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [[CameraManager cameraManager] params:@"5_sec" type:@"timelapse_video"];
    for (UITableViewCell * cell in self.timeLapseCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        timeLapseCheck = YES;
...
- (IBAction)timeLaps4Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [[CameraManager cameraManager] params:@"10_sec" type:@"timelapse_video"];
    for (UITableViewCell * cell in self.timeLapseCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        timeLapseCheck = YES;
...
- (IBAction)timeLaps5Button:(UIButton *)sender {
    ...
    [self checkFieldOfView];
    [[CameraManager cameraManager] params:@"30_sec" type:@"timelapse_video"];
    for (UITableViewCell * cell in self.timeLapseCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        timeLapseCheck = YES;
...
- (IBAction)videoClip1Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"1_min" type:@"video_length"];
    for (UITableViewCell * cell in self.videoClipCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoClipCheck = YES;
...
- (IBAction)videoClip2Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"2_min" type:@"video_length"];
    for (UITableViewCell * cell in self.videoClipCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoClipCheck = YES;
...
- (IBAction)videoClip3Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"3_min" type:@"video_length"];
    for (UITableViewCell * cell in self.videoClipCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoClipCheck = YES;
...
- (IBAction)videoClip4Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"5_min" type:@"video_length"];
    for (UITableViewCell * cell in self.videoClipCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoClipCheck = YES;
...
- (IBAction)videoClip5Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"10_min" type:@"video_length"];
    for (UITableViewCell * cell in self.videoClipCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoClipCheck = YES;
...
- (IBAction)videoClip6Button:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"continuous" type:@"video_length"];
    for (UITableViewCell * cell in self.videoClipCollection) {
        [self cell:cell setHidden:YES];
        [self reloadDataAnimated:YES];
        videoClipCheck = YES;
...
- (IBAction)imageRotationOneButton:(UIButton *)sender {
    imageRotationLabel.text = @"normal";
     ...
     [[CameraManager cameraManager] params:@"normal" type:@"rotate_video"];
...
- (IBAction)imageRotationTwoButton:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"180 degree" type:@"rotate_video"];
...
- (IBAction)imageRotationThreeButton:(UIButton *)sender {
    ...
    [[CameraManager cameraManager] params:@"auto rotation" type:@"rotate_video"];
...
- (IBAction)resetAllSettinsButton:(UIButton *)sender {
    
    self.resetSettingsMode1Label.backgroundColor = [UIColor colorWithRed:166/256.0 green:166/256.0 blue:166/256.0 alpha:1];
...
//            [[CameraManager cameraManager] resetAllSettings];
        [[CameraManager cameraManager] resetAllSettings:@"2560x1080 30P 21:9" type:@"video_resolution"];
        [[CameraManager cameraManager] resetAllSettings:@"on" type:@"audio"];
        [[CameraManager cameraManager] resetAllSettings:@"on" type:@"video_timestamp"];
        [[CameraManager cameraManager] resetAllSettings:@"normal" type:@"rotate_video"];
        [[CameraManager cameraManager] resetAllSettings:@"on" type:@"loop_record"];
            
        [[CameraManager cameraManager] resetAllSettings:@"S.Fine" type:@"video_quality"];
        [[CameraManager cameraManager] resetAllSettings:@"on" type:@"wdr"];
        [[CameraManager cameraManager] resetAllSettings:@"155" type:@"fleld_view"];
        [[CameraManager cameraManager] resetAllSettings:@"off" type:@"timelapse_video"];
...
-(void)resetSettingsMode1ButtonHighlight:(UIButton*) button {
    
    self.resetSettingsMode1Label.backgroundColor = [UIColor colorWithRed:72/256.0 green:216/256.0 blue:183/256.0 alpha:1];
}

-(void)resetSettingsMode1Button:(UIButton*) button {
